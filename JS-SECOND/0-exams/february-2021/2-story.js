class Story {
    ...
    like(username) {
        ...
        if (username === this.creator) { //? if username don't exist will return undefined --> currUser[0] === this.creator
    ...
    comment(username, content, id) {
        ...
        let replyId = `${currComment[0].id}.${currComment[0].replies.length + 1}`
    ...
    toString(type) {
        ...
        let result = this._comments.sort(sorting[type]).reduce((acc, e) => {
            acc.push(`-- ${e.id}. ${e.username}: ${e.content}`);
            if (e['replies']) {
                e.replies.sort(sorting[type]).forEach(r => acc.push(`--- ${r.id}. ${r.username}: ${r.content}`))
            }
            return acc
        }, [])
        result.unshift(`Title: ${this.title}`, `Creator: ${this.creator}`, `Likes: ${this._likes.length}`, 'Comments:')
        return result.join('\n');
    }
}
